...
class PlayUITest {
    ...
    fun testPlay() {
        ...
        Espresso.onView(ViewMatchers.withId(R.id.button_vsPly))
            .check(ViewAssertions.matches(ViewMatchers.isDisplayed()))
            .perform(ViewActions.click())
        ...
        Espresso.onView(ViewMatchers.withId(R.id.tableroLayout)).perform(
            ViewActions.actionWithAssertions(object : ViewAction {
                ...
                override fun perform(uiController: UiController?, view: View?) {
                    val tableroLayout = view as LinearLayout
                    val fila = tableroLayout.getChildAt(1) as LinearLayout
                    val ficha = fila.getChildAt(1)
                    ficha.performClick()
                }
            })
        )
